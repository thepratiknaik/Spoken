...
{
	if (preg_match('/[^A-Za-z0-9]{5,20}$/i', $username)) {
		return true;
	} else {
		return false;
	}
...
mysqli_query($db,"update `users` 
			set `ckey`= '', `ctime`= '' 
			where `id`='$_SESSION[user_id]' OR  `id` = '$_COOKIE[user_id]'") or die(mysqli_error($db));
}			
...
function checkData_entry() {

if($_SESSION['user_level'] == DATA_ENTRY_LEVEL) {
return 1;
} else { return 0 ;
}
