<?php
require "includes//stm_navbar_cys.php";
?>
...
// FOR SUM OF APPEARED
$query = "SELECT SUM(appeared) AS sum1 FROM `stm_cys_result` WHERE year = '$year_select' ";
$query_result1 = mysqli_query($conn, $query);
while ($row = mysqli_fetch_assoc($query_result1)) {
    $output1 = "" . " " . $row['sum1'];
}
// FOR SUM OF PASSED
$query = "SELECT SUM(passed) AS sum2 FROM `stm_cys_result` WHERE year = '$year_select' ";
$query_result2 = mysqli_query($conn, $query);
while ($row = mysqli_fetch_assoc($query_result2)) {
    $output2 = "" . " " . $row['sum2'];
}
// FOR SUM OF FAILED
$query = "SELECT SUM(failed) AS sum3 FROM `stm_cys_result` WHERE year = '$year_select' ";
...
$sql = "SELECT * FROM `stm_cys_result` where year = '$year_select'";
...
<head>
    ...
    <title>CYBER SECURITY DEPARTMENT RESULT</title>
